...
function amazonMusicRuntime(busDuration, songDurations) {
  let expected = [];
  let stopTime = busDuration - 30;
  let i = 0;
  while(i<songDurations.length){
    let j = i+1;
    while(j<songDurations.length){
      if(songDurations[i]+songDurations[j] != stopTime){
        j++;
      }else{
        expected.push(songDurations[i])
        expected.push(songDurations[j])
      }
    }
    i++
  }
